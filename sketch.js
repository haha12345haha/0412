...
function setup() {
  ...
  for (let i = 0; i < buttonLabels.length; i++) {
    ...
    btn.mouseOver(() => btn.style('background-color', '#ffbc42')); // 滑鼠移入變為淡藍色
    ...
    if (buttonLabels[i] === "作品集") {
      ...
      btn.mousePressed(() => showPopup("https://cfchen58.synology.me/%E7%A8%8B%E5%BC%8F%E8%A8%AD%E8%A8%882024/B2/week3/20250303_104548.mp4")); // 教學影片連結
...
function draw() {
  background('#c8b6ff'); // 設定背景顏色為 b9fbc0 (RGB: 185, 251, 192)
...
function toggleDropdown(x, y) {
  ...
  if (subButtons.length > 0) {
    ...
    // 建立子選項按鈕
    let subButtonLabels = ["作品1", "作品2", "作品3"]; // 更新子選單
    ...
    for (let i = 0; i < subButtonLabels.length; i++) {
      ...
      if (subButtonLabels[i] === "作品1") {
        btn.mousePressed(() => showPopup("https://haha12345haha.github.io/20250303_1/")); 
      } else if (subButtonLabels[i] === "作品2") {
        btn.mousePressed(() => showPopup("https://haha12345haha.github.io/0310/")); 
      } else if (subButtonLabels[i] === "作品3") {
        btn.mousePressed(() => showPopup("https://haha12345haha.github.io/04122/")); 
      }
      subButtons.push(btn);
    }
  }
...
function showIntroduction() {
  popupWindow.html('<p style="font-size: 28px; font-weight: bold; color:rgb(17, 11, 113); text-align: center; line-height: 150%;">我是蔣采緹，就讀淡江大學教育科技學系二年級</p>');
  ...
  popupWindow.style('border', '4px solid rgb(175, 53, 227)'); 
  popupWindow.style('border-radius', '10px'); // 改為長方形，圓角為 10px
...
function generateQuiz() {
  const questions = [
    {
      question: "1. 以下哪個是用於定義函式的關鍵字？",
      options: ["function", "method", "define", "procedure"],
      correct: 0
    },
    {
    ...
    {
      question: "3. 以下哪個是用於迴圈的關鍵字？",
      options: ["for", "repeat", "loop", "iterate"],
      correct: 0
    },
    {
      question: "4. 在 CSS 中，如何設定背景顏色？",
      options: ["background-color", "bg-color", "color", "background"],
      correct: 0
    },
    {
      question: "5. JavaScript 中如何取得 HTML 元素的 ID？",
      options: ["getElementById", "querySelector", "getId", "findElement"],
      correct: 0
    }
  ];
